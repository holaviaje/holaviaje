...
public interface IPostApplication
{
    ...
    Task<OneOf<PostViewModel, ErrorModel>> CreateUploadLinkAsync(long postId, UploadLinkModel model, long userId, CancellationToken cancellationToken = default);
    Task<OneOf<PostViewModel, ErrorModel>> DeleteMediaFileAsync(long postId, string fileId, long userId, CancellationToken cancellationToken = default);
